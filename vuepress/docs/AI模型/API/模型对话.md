...
curl_setopt_array($ch, [
    CURLOPT_URL => 'http://api.crmeb.net/api/v2/chat/conversation',
...
HttpRequest request = HttpRequest.newBuilder()
        .uri(URI.create("http://api.crmeb.net/api/v2/chat/conversation"))
...
(async () => {
    const response = await fetch('http://api.crmeb.net/api/v2/chat/conversation', {
        method: 'POST',
        headers: {
            'Authorization': 'Bearer-' + token,
            'Content-Type': 'application/json'
...
func main() {
	...
	req, _ := http.NewRequest("POST", "http://api.crmeb.net/api/v2/chat/conversation", strings.NewReader(`{"message":"xxx","stream":1,"assistant_message":[]}`))

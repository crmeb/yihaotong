...
curl_setopt_array($ch, [
    CURLOPT_URL => 'http://api.crmeb.net/api/v2/copy/goods',
...
HttpRequest request = HttpRequest.newBuilder()
        .uri(URI.create("http://api.crmeb.net/api/v2/copy/goods"))
...
(async () => {
    const response = await fetch('http://api.crmeb.net/api/v2/copy/goods', {
        method: 'POST',
        headers: {
            'Authorization': 'Bearer-' + token,
            'Content-Type': 'application/json'
...
func main() {
	...
	req, _ := http.NewRequest("POST", "http://api.crmeb.net/api/v2/copy/goods", strings.NewReader(`{"url":"https://www.example.com"}`))

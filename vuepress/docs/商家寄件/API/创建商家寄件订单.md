...
curl_setopt_array($ch, [
    CURLOPT_URL => 'http://api.crmeb.net/api/v2/shipment/create_order',
...
HttpRequest request = HttpRequest.newBuilder()
        .uri(URI.create("http://api.crmeb.net/api/v2/shipment/create_order"))
...
(async () => {
    const response = await fetch('http://api.crmeb.net/api/v2/shipment/create_order', {
        method: 'POST',
        headers: {
            'Authorization': 'Bearer-' + token,
            'Content-Type': 'application/json'

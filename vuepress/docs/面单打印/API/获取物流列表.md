...
curl_setopt_array($ch, [
    CURLOPT_URL => 'http://api.crmeb.net/api/v2/expr/express?appid=xxx&number=xxx&status=xxx&created_at=xxx&page=1&limit=20',
...
HttpRequest request = HttpRequest.newBuilder()
        .uri(URI.create("http://api.crmeb.net/api/v2/expr/express?appid=xxx&number=xxx&status=xxx&created_at=xxx&page=1&limit=20"))
...
(async () => {
    const response = await fetch('http://api.crmeb.net/api/v2/expr/express?appid=xxx&number=xxx&status=xxx&created_at=xxx&page=1&limit=20', {
        headers: {
            'Authorization': 'Bearer-' + token
        }
    });
...
func main() {
	...
	req, _ := http.NewRequest("GET", "http://api.crmeb.net/api/v2/expr/express?appid=xxx&number=xxx&status=xxx&created_at=xxx&page=1&limit=20", nil)

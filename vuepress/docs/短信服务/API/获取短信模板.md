...
curl_setopt_array($ch, [
    CURLOPT_URL => 'http://api.crmeb.net/api/v2/sms_v2/temps',
...
HttpRequest request = HttpRequest.newBuilder()
        .uri(URI.create("http://api.crmeb.net/api/v2/sms_v2/temps"))
...
(async () => {
    const response = await fetch('http://api.crmeb.net/api/v2/sms_v2/temps', {
        method: 'POST',
        headers: {
            'Authorization': 'Bearer-' + token,
            'Content-Type': 'application/json'
...
func main() {
	...
	req, _ := http.NewRequest("POST", "http://api.crmeb.net/api/v2/sms_v2/temps", strings.NewReader(`{"temp_type":1,"page":1,"limit":20}`))
